...
ReconstructionEngine_sequentialSfM::ReconstructionEngine_sequentialSfM(const SfMData& sfmData, const Params& params,
    ...
    else
    {
        ...
        if(_params.useLocalBundleAdjustment)
        {
            ...
            if(!reconstructedViews.empty())
            {
                ...
                _localStrategyGraph->updateGraphWithNewViews(_sfmData, _map_tracksPerView, reconstructedViews,
                                                             _params.kMinNbOfMatches);
...
std::size_t ReconstructionEngine_sequentialSfM::fuseMatchesIntoTracks()
{
...

    {
        ...
        computeTracksPyramidPerView(_map_tracksPerView, _map_tracks, _sfmData.views, *_featuresPerView,
                                    _params.pyramidBase, _params.pyramidDepth, _map_featsPyramidPerView);
    ...
    do
    {
        ...
        while(findNextBestViews(bestViewCandidates, remainingViewIds))
        {
            ...
            std::set<IndexT> newReconstructedViews =
                resection(resectionId, bestViewCandidates, prevReconstructedViews, remainingViewIds);
...
void ReconstructionEngine_sequentialSfM::triangulate(const std::set<IndexT>& prevReconstructedViews,
...
bool ReconstructionEngine_sequentialSfM::bundleAdjustment(std::set<IndexT>& newReconstructedViews, bool isInitialPair)
{
    ...
    if(_params.useLocalBundleAdjustment)
        _localStrategyGraph->updateGraphWithNewViews(_sfmData, _map_tracksPerView, newReconstructedViews,
                                                     _params.kMinNbOfMatches);
    ...
    if(enableLocalStrategy)
    {
        ...
        // use the graph-distances to assign a state (Refine, Constant & Ignore) for each parameter (poses, intrinsics &
        // landmarks)
        _localStrategyGraph->convertDistancesToStates(_sfmData);

        const std::size_t nbRefinedPoses =
            _localStrategyGraph->getNbPosesPerState(BundleAdjustment::EParameterState::REFINED);
        const std::size_t nbConstantPoses =
            _localStrategyGraph->getNbPosesPerState(BundleAdjustment::EParameterState::CONSTANT);
        ...
        {
            throw std::runtime_error("The local bundle adjustment refinement has not been done: the new cameras are "
                                     "not connected to the rest of the graph.");
        }
    }
    ...
    do
    {
        ...
        eraseUnstablePosesAndObservations(this->_sfmData, _params.minPointsPerPose, _params.minTrackLength,
                                          &removedViewsIdIteration);
        ...
        ALICEVISION_LOG_INFO("Bundle adjustment iteration: " << iteration << " took "
                                                             << std::chrono::duration_cast<std::chrono::milliseconds>(
                                                                    std::chrono::steady_clock::now() - chronoItStart)
                                                                    .count()
                                                             << " msec.");
        ++iteration;
...
bool ReconstructionEngine_sequentialSfM::makeInitialPair3D(const Pair& currentPair, int step)
{
    ...
    for(aliceVision::track::TracksMap::const_iterator iterT = commonTracks.begin(); iterT != commonTracks.end();
        ++iterT, ++cptIndex)
    {
    ...
    relativePoseInfo.found_residual_precision = std::max(relativePoseInfo.found_residual_precision, 1.0);
    {
        ...
        if(!_htmlLogFile.empty())
        {
            ...
            jsxGraph.addLine(relativePoseInfo.found_residual_precision, 0, relativePoseInfo.found_residual_precision,
...
bool ReconstructionEngine_sequentialSfM::getBestInitialImagePairs(std::vector<Pair>& out_bestImagePairs,
                                                                  IndexT filterViewId) const
{
...
double ReconstructionEngine_sequentialSfM::computeObservationsLengthsHistogram(Histogram<double>* histo) const
{
    ...
    ALICEVISION_LOG_INFO("Landmarks observations length min: " << stats.min << ", mean: " << stats.mean << ", median: "
                                                               << stats.median << ", max: " << stats.max);
...
double ReconstructionEngine_sequentialSfM::computeLandmarksPerViewHistogram(Histogram<double>* histo) const
{
    ...
    std::transform(_sfmData.getLandmarks().begin(), _sfmData.getLandmarks().end(),
                   std::inserter(landmarksId, landmarksId.begin()), stl::RetrieveKey());
    ...
    for(const auto& viewIt : _sfmData.getViews())
    {
        ...
        aliceVision::track::TrackIdSet viewLandmarksIds;
        {
            ...
            std::set_intersection(viewTracksIds.begin(), viewTracksIds.end(), landmarksId.begin(), landmarksId.end(),
    ...
    ALICEVISION_LOG_INFO("Landmarks per view min: " << stats.min << ", mean: " << stats.mean
...
std::size_t
ReconstructionEngine_sequentialSfM::computeCandidateImageScore(IndexT viewId,
...
bool ReconstructionEngine_sequentialSfM::computeResection(const IndexT viewId, ResectionData& resectionData)
{
    ...
    tracksUtilsMap::getFeatureIdInViewPerTrack(_map_tracks, resectionData.tracksId, viewId, &resectionData.featuresId);
        ...
        iterfeatId != resectionData.featuresId.end(); ++iterfeatId, ++iterTrackId, ++cpt)
    {
        ...
        resectionData.pt2D.col(cpt) =
            _featuresPerView->getFeatures(viewId, descType)[iterfeatId->second].coords().cast<double>();
        resectionData.vec_descType.at(cpt) = descType;
    }

    // C. Do the resectioning: compute the camera pose.
    ALICEVISION_LOG_INFO("[" << _sfmData.getValidViews().size() + 1 << "/" << _sfmData.getViews().size()
                             << "] Robust Resection of view: " << viewId);
    ...
    const bool bResection = sfm::SfMLocalizer::Localize(Pair(view_I->getWidth(), view_I->getHeight()),
                                                        ...
                                                        resectionData.pose, _params.localizerEstimator);
...
void ReconstructionEngine_sequentialSfM::triangulate_multiViewsLORANSAC(
...
#pragma omp critical
                    ...
                    if(angle > _params.minAngleForTriangulation && poseI.depth(X_euclidean) > 0 &&
