...
#ifdef _MSC_VER
#pragma warning(                                                                                                       \
    once : 4267) // warning C4267: 'argument' : conversion from 'size_t' to 'const int', possible loss of data
#endif
 ...
 */
void computeTracksPyramidPerView(const track::TracksPerView& tracksPerView, const track::TracksMap& map_tracks,
                                 const Views& views, const feature::FeaturesPerView& featuresProvider,
                                 const std::size_t pyramidBase, const std::size_t pyramidDepth,
...
ReconstructionEngine_sequentialSfM::ReconstructionEngine_sequentialSfM(const SfMData& sfm_data,
...
std::size_t ReconstructionEngine_sequentialSfM::fuseMatchesIntoTracks()
{
...

    {
        ...
        computeTracksPyramidPerView(_map_tracksPerView, _map_tracks, _sfmData.views, *_featuresPerView, _pyramidBase,
                                    _pyramidDepth, _map_featsPyramidPerView);
...
void ReconstructionEngine_sequentialSfM::createInitialReconstruction(
...
void ReconstructionEngine_sequentialSfM::remapLandmarkIdsToTrackIds()
{
    ...
    for(const auto& trackPair : _map_tracks)
    {
        ...
        {
            const ObsToLandmark::const_iterator it =
                obsToLandmark.find(ObsKey(featView.first, featView.second, track.descType));
    ...
    ALICEVISION_LOG_INFO("Landmark ids to track ids reampping: "
                         << std::endl
...
double ReconstructionEngine_sequentialSfM::incrementalReconstruction()
{
    ...
    while(findNextBestViews(bestViewIds, viewIds))
    {
        ALICEVISION_LOG_INFO("Update Reconstruction:" << std::endl
                                                      ...
                                                      << "\t- # images in the resection group: " << bestViewIds.size()
                                                      << std::endl
...
void ReconstructionEngine_sequentialSfM::updateReconstruction(IndexT resectionId,
...
#pragma omp parallel for
    for(int i = 0; i < bestViewIds.size(); ++i)
    {
        ...
        if(view.isPartOfRig())
        {
            ...
            {
                ALICEVISION_LOG_DEBUG("Resection of image "
                                      << i << " was skipped." << std::endl
...
#pragma omp critical
    ...
    ALICEVISION_LOG_DEBUG("Resection of " << bestViewIds.size() << " new images took "
    ...
    std::set<IndexT> newReconstructedViews;
    {
        ...
        std::set_difference(reconstructedViews.begin(), reconstructedViews.end(), prevReconstructedViews.begin(),
    ...
    ALICEVISION_LOG_DEBUG("Triangulation of the " << newReconstructedViews.size() << " newly reconstructed views took "
                                                  << std::chrono::duration_cast<std::chrono::milliseconds>(
                                                         std::chrono::steady_clock::now() - chrono_start)
                                                         .count()
                                                  << " msec.");
    ...
    if(imageAdded)
    {
        if((resectionId % 3) == 0)
        {
            ...
            Save(_sfmData, (fs::path(_outputFolder) / (os.str() + _sfmdataInterFileExtension)).string(),
                 _sfmdataInterFilter);
        ...
        do
        {
            ...
            ALICEVISION_LOG_DEBUG("Resection index: " << resectionId
                                                      << ", bundle iteration: " << bundleAdjustmentIteration << " took "
                                                      << std::chrono::duration_cast<std::chrono::milliseconds>(
                                                             std::chrono::steady_clock::now() - chrono2_start)
                                                             .count()
                                                      << " msec.");
            ++bundleAdjustmentIteration;
        } while(removeOutliers(_maxReprojectionError) > nbOutliersThreshold);

        ALICEVISION_LOG_DEBUG("Bundle adjustment with " << bundleAdjustmentIteration << " iterations took "
                                                        << std::chrono::duration_cast<std::chrono::milliseconds>(
                                                               std::chrono::steady_clock::now() - chrono_start)
                                                               .count()
                                                        << " msec.");
        ...
        bool contentRemoved =
            eraseUnstablePosesAndObservations(this->_sfmData, _minPointsPerPose, _minTrackLength, &removedPosesId);
            ...
            for(const auto& x : _sfmData.getViews())
            {
                if(removedPosesId.find(x.second->getPoseId()) != removedPosesId.end())
                {
                    if(!_sfmData.isPoseAndIntrinsicDefined(x.second->getViewId()))
                        ...
                        ALICEVISION_LOG_WARNING("The view #" << x.second->getViewId()
                                                             << " is set as Removed while it is still in the scene.");
                }
            }
        ...
        ALICEVISION_LOG_DEBUG(
            "eraseUnstablePosesAndObservations took "
            << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - chrono_start)
                   .count()
            << " msec.");
    }
    ...
    ALICEVISION_LOG_INFO("Update Reconstruction complete: " << std::endl
                                                            << "\t- # cameras calibrated: "
                                                            << _sfmData.getPoses().size() << std::endl
...
void ReconstructionEngine_sequentialSfM::exportStatistics(double reconstructionTime)
{
    ...
    ALICEVISION_LOG_INFO("Structure from Motion statistics:"
                         << std::endl
                                                            ...
                                                            const std::set<IndexT>& remainingViewIds) const
{
    ...
    std::transform(_sfmData.getLandmarks().begin(), _sfmData.getLandmarks().end(),
                   std::inserter(reconstructed_trackId, reconstructed_trackId.begin()), stl::RetrieveKey());
...
#pragma omp parallel for
    for(int i = 0; i < remainingViewIds.size(); ++i)
    {
        ...
        std::set_intersection(set_tracksIds.begin(), set_tracksIds.end(), reconstructed_trackId.begin(),
                              reconstructed_trackId.end(), std::back_inserter(vec_trackIdForResection));
...
#pragma omp critical
    ...
    std::sort(
        out_connectedViews.begin(), out_connectedViews.end(),
        [](const ViewConnectionScore& t1, const ViewConnectionScore& t2) { return std::get<2>(t1) > std::get<2>(t2); });
    return !out_connectedViews.empty();
}

bool ReconstructionEngine_sequentialSfM::findNextBestViews(std::vector<IndexT>& out_selectedViewIds,
                                                           const std::set<IndexT>& remainingViewIds) const
{
    ...
    for(std::size_t i = 1; i < vec_viewsScore.size() &&
    ...
    if(_sfmData.getPoses().size() < nbFirstUnstableCameras && !out_selectedViewIds.empty())
    {
        ...
        ALICEVISION_LOG_DEBUG("findNextBestViews: beginning of the incremental SfM"
                              ...
                              << "\t- # unstable poses : " << _sfmData.getPoses().size() << " / "
                              << nbFirstUnstableCameras << std::endl);
    ...
    ALICEVISION_LOG_DEBUG("Find next best views took: " << std::chrono::duration_cast<std::chrono::milliseconds>(
                                                               std::chrono::steady_clock::now() - chrono_start)
                                                               .count()
                                                        << " msec\n"
                                                           "\t# images : "
                                                        ...
                                                        << "\n"
                                                           "\t- scores: from "
                                                        << std::get<2>(vec_viewsScore.front()) << " to "
                                                        << std::get<2>(vec_viewsScore[out_selectedViewIds.size() - 1])
                                                        << " (threshold was " << scoreThreshold
                                                        << ")\n"
                                                           "\t- features: from "
                                                        << std::get<1>(vec_viewsScore.front()) << " to "
                                                        << std::get<1>(vec_viewsScore[out_selectedViewIds.size() - 1])
                                                        << " (threshold was " << minPointsThreshold << ")");

    return (!out_selectedViewIds.empty());
}

bool ReconstructionEngine_sequentialSfM::makeInitialPair3D(const Pair& current_pair, int step)
{
    ...
    if(iterIntrinsicI == _sfmData.getIntrinsics().end() || iterIntrinsicJ == _sfmData.getIntrinsics().end())
    {
    ...
    track::tracksUtilsMap::getCommonTracksInImagesFast(set_imageIndex, _map_tracks, _map_tracksPerView,
                                                       map_tracksCommon);
    ...
    for(aliceVision::track::TracksMap::const_iterator iterT = map_tracksCommon.begin(); iterT != map_tracksCommon.end();
        ++iterT, ++cptIndex)
    {
    ...
    if(!robustRelativePose(camI->K(), camJ->K(), xI, xJ, relativePose_info, imageSizeI, imageSizeJ, 4096))
    {
        ALICEVISION_LOG_WARNING("Robust estimation failed to compute E for this pair");
        return false;
    }
    ALICEVISION_LOG_DEBUG("A-Contrario initial pair residual: " << relativePose_info.found_residual_precision);
...

    {
        ...
        float camRotNextDeg = offsetCamRot - rotAngle;
        ...
        if(step > 0)
        {
            ...
            camRotDeg = offsetCamRot - rotAngle * (step * 2);
            camRotNextDeg = offsetCamRot - rotAngle * (step * 2) - rotAngle;
        }
        ...
        Mat3 rotOffsetX = Mat3::Identity();

        Mat3 rotLocal = Mat3::Identity();
        ...
        Mat3 rotOrbit = Mat3::Identity();
        Mat3 rotNextOrbit = Mat3::Identity();
		...
		float offsetXRad = 15 * M_PI / 180.0;

        rotOffsetX(4) = std::cos(offsetXRad);
        rotOffsetX(5) = std::sin(offsetXRad);
        rotOffsetX(7) = -std::sin(offsetXRad);
        rotOffsetX(8) = std::cos(offsetXRad);
		...
		float offsetYRad = 1 * M_PI / 180.0;

        rotYoffest(0) = std::cos(M_PI_2 + offsetYRad);
        rotYoffest(2) = std::sin(M_PI_2 + offsetYRad);
        rotYoffest(6) = -std::sin(M_PI_2 + offsetYRad);
        rotYoffest(8) = std::cos(M_PI_2 + offsetYRad);
        ...
        rotOrbit(0) = std::cos(pi);
        rotOrbit(2) = std::sin(pi);
        rotOrbit(6) = -std::sin(pi);
        rotOrbit(8) = std::cos(pi);

        rotNextOrbit(0) = std::cos(piNext);
        rotNextOrbit(2) = std::sin(piNext);
        rotNextOrbit(6) = -std::sin(piNext);
        rotNextOrbit(8) = std::cos(piNext);

        float z = radie * std::sin(theta) * 1; // std::sin(-M_PI_4);
        float x = 0;                           // radie * std::sin(theta) * std::cos(pi);
        float y = radie * std::cos(theta);

        float scaleFact = 0.0131;
        float minRadie = 10;
        /*
        -1057.04059201079 131.793582115810 -557.664837835153
        -971.619828917655 121.940272433841 -687.874674645891
        -877.843362548072 105.966429733722 -792.708098184077
        -761.352952711828 94.1279191001301 -901.463225895317
        -643.612227925782 77.1416090266953 -988.688240016276
        -517.555027222001 62.7455547197297 -1063.38057319070
        -382.573845129433 43.6214157452838 -1116.63104083780
        -244.154165137669 23.8461454660322 -1156.64231369488
        -110.170518338464 1.79682659477223 -1180.51946320981
         115.564125677598 55.5146271941007 -1190.96020549383 */
        ...
        {
            x = 151.788 * scaleFact + minRadie;
            y = 551.082 * scaleFact;
            z = 1027.4745 * scaleFact + minRadie;

            orientation(0) = 0.41233;
            orientation(1) = 0.067896;
            orientation(2) = 0.9085;
            orientation(3) = 0.0083858;
            orientation(4) = 0.99689;
            orientation(5) = -0.078308;
            orientation(6) = -0.911;
            orientation(7) = 0.039907;
            orientation(8) = 0.41048;
        }
        else if(step >= (camerasPairsPerRow * 1) && step < camerasPairsPerRow * 2)
        {
            x = 164.793 * scaleFact + minRadie;
            y = 753.812 * scaleFact;
            z = 954.7422 * scaleFact + minRadie;

            orientation(0) = 0.56182;
            orientation(1) = 0.085198;
            orientation(2) = 0.82286;
            orientation(3) = -0.025804;
            orientation(4) = 0.996;
            orientation(5) = -0.085507;
            orientation(6) = -0.82686;
            orientation(7) = 0.026807;
            orientation(8) = 0.56177;
        }
        else if(step >= (camerasPairsPerRow * 2) && step < camerasPairsPerRow * 3)
        {
            x = 172.6037 * scaleFact + minRadie;
            y = 884.3197 * scaleFact;
            z = 869.8458 * scaleFact + minRadie;

            orientation(0) = 0.66337;
            orientation(1) = 0.075693;
            orientation(2) = 0.74445;
            orientation(3) = -0.01352;
            orientation(4) = 0.99592;
            orientation(5) = -0.089213;
            orientation(6) = -0.74817;
            orientation(7) = 0.049117;
            orientation(8) = 0.66169;
        }
        else if(step >= (camerasPairsPerRow * 3) && step < camerasPairsPerRow * 4)
        {
            x = 172.1237 * scaleFact + minRadie;
            y = 1006.6858 * scaleFact;
            z = 764.3152 * scaleFact + minRadie;

            orientation(0) = 0.74386;
            orientation(1) = 0.13143;
            orientation(2) = 0.65529;
            orientation(3) = -0.095978;
            orientation(4) = 0.99132;
            orientation(5) = -0.089871;
            orientation(6) = -0.66141;
            orientation(7) = 0.0039574;
            orientation(8) = 0.75001;
        }
        else if(step >= (camerasPairsPerRow * 4) && step < camerasPairsPerRow * 5)
        {
            x = 168.7753 * scaleFact + minRadie;
            y = 1119.7959 * scaleFact;
            z = 632.6826 * scaleFact + minRadie;

            orientation(0) = 0.83136;
            orientation(1) = 0.11403;
            orientation(2) = 0.5439;
            orientation(3) = -0.079918;
            orientation(4) = 0.99308;
            orientation(5) = -0.086041;
            orientation(6) = -0.54995;
            orientation(7) = 0.028064;
            orientation(8) = 0.83473;
        }
        else if(step >= (camerasPairsPerRow * 5) && step < camerasPairsPerRow * 6)
        {
            x = 163.399 * scaleFact + minRadie;
            y = 1202.6958 * scaleFact;
            z = 498.2141 * scaleFact + minRadie;

            orientation(0) = 0.89342;
            orientation(1) = 0.080994;
            orientation(2) = 0.44185;
            orientation(3) = -0.050781;
            orientation(4) = 0.99552;
            orientation(5) = -0.079805;
            orientation(6) = -0.44634;
            orientation(7) = 0.048862;
            orientation(8) = 0.89353;
        }
        else if(step >= (camerasPairsPerRow * 6) && step < camerasPairsPerRow * 7)
        {
            x = 157.235 * scaleFact + minRadie;
            y = 1271.5612 * scaleFact;
            z = 342.3299 * scaleFact + minRadie;

            orientation(0) = 0.94029;
            orientation(1) = 0.10195;
            orientation(2) = 0.32474;
            orientation(3) = -0.082015;
            orientation(4) = 0.99384;
            orientation(5) = -0.074546;
            orientation(6) = -0.33034;
            orientation(7) = 0.043462;
            orientation(8) = 0.94286;
        }
        else if(step >= (camerasPairsPerRow * 7) && step < camerasPairsPerRow * 8)
        {
            x = 148.3301 * scaleFact + minRadie;
            y = 1315.6373 * scaleFact;
            z = 203.0108 * scaleFact + minRadie;

            orientation(0) = 0.97312;
            orientation(1) = 0.088881;
            orientation(2) = 0.21247;
            orientation(3) = -0.07563;
            orientation(4) = 0.9947;
            orientation(5) = -0.069715;
            orientation(6) = -0.21754;
            orientation(7) = 0.051772;
            orientation(8) = 0.97468;
        }
        else if(step >= (camerasPairsPerRow * 8) && step < camerasPairsPerRow * 9)
        {
            x = 139.941 * scaleFact + minRadie;
            y = 1339.5803 * scaleFact;
            z = 49.9175 * scaleFact + minRadie;

            orientation(0) = 0.99169;
            orientation(1) = 0.090742;
            orientation(2) = 0.091195;
            orientation(3) = -0.085517;
            orientation(4) = 0.99455;
            orientation(5) = -0.059665;
            orientation(6) = -0.096112;
            orientation(7) = 0.051371;
            orientation(8) = 0.99404;
        }
        else if(step >= (camerasPairsPerRow * 9) && step < camerasPairsPerRow * 10)
        {
            x = 129.1346 * scaleFact + minRadie;
            y = 1341.5715 * scaleFact;
            z = -92.0801 * scaleFact + minRadie;

            orientation(0) = 0.99689;
            orientation(1) = 0.0768;
            orientation(2) = -0.017605;
            orientation(3) = -0.077625;
            orientation(4) = 0.99561;
            orientation(5) = -0.052354;
            orientation(6) = 0.013507;
            orientation(7) = 0.053558;
            orientation(8) = 0.99847;
        }
        else if(step >= (camerasPairsPerRow * 10) && step < camerasPairsPerRow * 11)
        {
            x = 115.2575 * scaleFact + minRadie;
            y = 1324.6278 * scaleFact;
            z = -245.1985 * scaleFact + minRadie;

            orientation(0) = 0.9872;
            orientation(1) = 0.095471;
            orientation(2) = -0.12779;
            orientation(3) = -0.10158;
            orientation(4) = 0.99393;
            orientation(5) = -0.042175;
            orientation(6) = 0.12298;
            orientation(7) = 0.054616;
            orientation(8) = 0.9909;
        }
        else if(step >= (camerasPairsPerRow * 11) && step < camerasPairsPerRow * 12)
        {
            x = 102.0487 * scaleFact + minRadie;
            y = 1296.4285 * scaleFact;
            z = -391.5647 * scaleFact + minRadie;

            orientation(0) = 0.96422;
            orientation(1) = 0.11237;
            orientation(2) = -0.24011;
            orientation(3) = -0.12349;
            orientation(4) = 0.99184;
            orientation(5) = -0.031713;
            orientation(6) = 0.23459;
            orientation(7) = 0.060229;
            orientation(8) = 0.97023;
        }
        else if(step >= (camerasPairsPerRow * 12) && step < camerasPairsPerRow * 13)
        {
            x = 85.9175 * scaleFact + minRadie;
            y = 1225.9805 * scaleFact;
            z = -570.4471 * scaleFact + minRadie;

            orientation(0) = 0.91605;
            orientation(1) = -0.13073;
            orientation(2) = 0.37917;
            orientation(3) = 0.11341;
            orientation(4) = 0.99123;
            orientation(5) = 0.067767;
            orientation(6) = -0.3847;
            orientation(7) = -0.019076;
            orientation(8) = 0.92284;
        }

        rotLocal = rotZoffest * rotYoffest * rotX;

        const Pose3& offsetRotX = Pose3(rotOffsetX, Vec3(1, 0, 0));

		

        // calculate init pose
        const Pose3& initPoseI = Pose3(orientation * rotYoffest, rotOffsetX * Vec3(x - 11, y, z));

        const Pose3& initPoseJ = Pose3(orientation * rotYoffest, rotOffsetX * Vec3(x - 11, y, z));

        // calculate the global rotation for first camera
        const Pose3& initPoseRot = Pose3(rotOrbit, Vec3(0, 1, 0));
        // calculate the global rotation for secondc camera
        const Pose3& initPoseRotNext = Pose3(rotNextOrbit, Vec3(0, 1, 0));
        ...
        Save(_sfmData, (fs::path(_outputFolder) / ("initialPair" + _sfmdataInterFileExtension)).string(),
             _sfmdataInterFilter);
        ...
        {
          ALICEVISION_LOG_WARNING("BA of initial pair " << current_pair.first << ", " << current_pair.second << "
        failed.");
        ...
        */
        ...
        if(!_htmlLogFile.empty())
        {
            ...
            _htmlDocStream->pushInfo(
                htmlMarkup("h3", "Initial triangulation - Residual of the robust estimation.<br>Thresholded at: " +
            ...
            jsxGraph.addLine(relativePose_info.found_residual_precision, 0, relativePose_info.found_residual_precision,
                             histoResiduals.GetHist().front());
...
bool ReconstructionEngine_sequentialSfM::getBestInitialImagePairs(std::vector<Pair>& out_bestImagePairs) const
{
    ...
    if(valid_views.size() < 2)
    {
        ALICEVISION_LOG_WARNING(
            "Failed to find an initial pair automatically. There is no view with valid intrinsics.");
        return false;
    }
...
#pragma omp critical
        ...
        for(aliceVision::track::TracksMap::const_iterator iterT = map_tracksCommon.begin();
            iterT != map_tracksCommon.end(); ++iterT, ++cptIndex)
        {
            auto iter = iterT->second.featPerView.begin();
        ...
        const bool relativePoseSuccess =
        ...
        const double imagePairScore =
            std::min(computeImageScore(I, validCommonTracksIds), computeImageScore(J, validCommonTracksIds));
...
double ReconstructionEngine_sequentialSfM::computeResidualsHistogram(Histogram<double>* histo) const
{
    ...
    for(const auto& track : _sfmData.getLandmarks())
    {
        ...
        for(const auto& obs : observations)
        {
            ...
            const std::shared_ptr<IntrinsicBase> intrinsic =
                _sfmData.getIntrinsics().find(view->getIntrinsicId())->second;
    ...
    ALICEVISION_LOG_DEBUG("ReconstructionEngine_sequentialSfM::ComputeResidualsMSE."
                          << std::endl
...
std::size_t ReconstructionEngine_sequentialSfM::computeImageScore(IndexT viewId,
                                                                  const std::vector<std::size_t>& trackIds) const
{
#ifdef ALICEVISION_NEXTBESTVIEW_WITHOUT_SCORE
...
bool ReconstructionEngine_sequentialSfM::computeResection(const IndexT viewIndex, ResectionData& resectionData)
{
    ...
    tracksUtilsMap::getFeatureIdInViewPerTrack(_map_tracks, resectionData.tracksId, viewIndex,
    ...
    for(std::vector<tracksUtilsMap::FeatureId>::const_iterator iterfeatId = resectionData.featuresId.begin();
        iterfeatId != resectionData.featuresId.end(); ++iterfeatId, ++iterTrackId, ++cpt)
    {
        ...
        resectionData.pt2D.col(cpt) =
            _featuresPerView->getFeatures(viewIndex, descType)[iterfeatId->second].coords().cast<double>();
    ...
    const bool bResection = sfm::SfMLocalizer::Localize(Pair(view_I->getWidth(), view_I->getHeight()),
                                                        resectionData.optionalIntrinsic.get(), resectionData,
                                                        resectionData.pose, _localizerEstimator);
    ...
    // We use a local scene with only the 3D points and the new camera.
    {
        ...
        if(pinhole_cam == nullptr)
            throw std::runtime_error("Intrinsic " + std::to_string(view_I->getIntrinsicId()) +
                                     " is not a Pinhole camera. This is not supported in the incremental pipeline.");
...
void ReconstructionEngine_sequentialSfM::updateScene(const IndexT viewIndex, const ResectionData& resectionData)
{
    ...
    for(std::size_t i = 0; i < resectionData.pt2D.cols(); ++i, ++iterTrackId)
    {
        ...
        if(residual.norm() < resectionData.error_max && resectionData.pose.depth(X) > 0)
        {
            ...
            _sfmData.structure[*iterTrackId].observations[viewIndex] =
                Observation(x, resectionData.featuresId[i].second);
        }
    }
}

bool ReconstructionEngine_sequentialSfM::checkChieralities(const Vec3& pt3D, const std::set<IndexT>& viewsId,
                                                           const SfMData& scene)
{
...
bool ReconstructionEngine_sequentialSfM::checkAngles(const Vec3& pt3D, const std::set<IndexT>& viewsId,
                                                     const SfMData& scene, const double& kMinAngle)
{
    for(const std::size_t& viewIdA : viewsId)
    {
        for(const std::size_t& viewIdB : viewsId)
        {
            ...
            {
                double angle_deg =
                    AngleBetweenRays(scene.getPose(*scene.getViews().at(viewIdA).get()).getTransform(),
                                     scene.getPose(*scene.getViews().at(viewIdB).get()).getTransform(), pt3D);
...
void ReconstructionEngine_sequentialSfM::getTracksToTriangulate(
    const std::set<IndexT>& previousReconstructedViews, const std::set<IndexT>& newReconstructedViews,
...
#pragma omp single nowait
            {
                ...
                std::set_intersection(
                    allViewsSharingTheTrack.begin(), allViewsSharingTheTrack.end(), allReconstructedViews.begin(),
...
void ReconstructionEngine_sequentialSfM::triangulateMultiViews_LORANSAC(
    SfMData& scene, const std::set<IndexT>& previousReconstructedViews, const std::set<IndexT>& newReconstructedViews)
{
    ...
    std::transform(mapTracksToTriangulate.begin(), mapTracksToTriangulate.end(),
                   std::inserter(setTracksId, setTracksId.begin()), stl::RetrieveKey());
...
#pragma omp parallel for
    for(int i = 0; i < setTracksId.size(); i++) // each track (already reconstructed or not)
    {
        ...
        if(observations.size() == 2)
        {
            ...
            const Vec2 xI =
                _featuresPerView->getFeatures(I, track.descType)[track.featPerView.at(I)].coords().cast<double>();
            const Vec2 xJ =
                _featuresPerView->getFeatures(J, track.descType)[track.featPerView.at(J)].coords().cast<double>();

            // -- Triangulate:
            TriangulateDLT(camI->get_projective_equivalent(poseI), camI->get_ud_pixel(xI),
                           camJ->get_projective_equivalent(poseJ), camI->get_ud_pixel(xJ), &X_euclidean);
            ...
            if(AngleBetweenRays(poseI, camI, poseJ, camJ, xI, xJ) < _minAngleForTriangulation ||
               poseI.depth(X_euclidean) < 0 || poseJ.depth(X_euclidean) < 0 ||
        ...
        else
        {
            ...
            std::vector<Mat34> Ps;                  // projective matrices (one per pose)
            {
                ...
                for(const IndexT& viewId : observations)
                {
                    ...
                    const Vec2 x_ud = cam->get_ud_pixel(
                        _featuresPerView->getFeatures(viewId, track.descType)[track.featPerView.at(viewId)]
                            .coords()
                            .cast<double>()); // undistorted 2D point
                    features(0, i) = x_ud(0);
                    features(1, i) = x_ud(1);
        ...
        if(isValidTrack)
        {
            ...
            {
                const Vec2 x = _featuresPerView->getFeatures(viewId, track.descType)[track.featPerView.at(viewId)]
                                   .coords()
                                   .cast<double>();
...
void ReconstructionEngine_sequentialSfM::triangulate(SfMData& scene, const std::set<IndexT>& previousReconstructedViews,
                                                     const std::set<IndexT>& newReconstructedViews)
{
    {
        std::vector<IndexT> intersection;
        std::set_intersection(newReconstructedViews.begin(), newReconstructedViews.end(),
                              previousReconstructedViews.begin(), previousReconstructedViews.end(),
...
#pragma omp parallel for schedule(dynamic)
    for(ptrdiff_t i = 0; i < static_cast<ptrdiff_t>(allReconstructedViews.size()); ++i)
    {
        ...
        for(IndexT indexNew : newReconstructedViews)
        {
            ...
            track::tracksUtilsMap::getCommonTracksInImagesFast(set_viewIndex, _map_tracks, _map_tracksPerView,
                                                               map_tracksCommonIJ);
...
#pragma omp critical
                    {
                        ...
                        if(landmark.observations.count(I) == 0)
                        {
                            ...
                            const double acThreshold =
                                (acThresholdIt != _map_ACThreshold.end()) ? acThresholdIt->second : 4.0;
...
#pragma omp critical
                    ...
                    const double& acThresholdI =
                        (acThresholdItI != _map_ACThreshold.end()) ? acThresholdItI->second : 4.0;
...
bool ReconstructionEngine_sequentialSfM::localBundleAdjustment(const std::set<IndexT>& newReconstructedViews)
{
    ...
    if(options.isLocalBAEnabled()) // Local Bundle Adjustment
    {
        ...
        // Use the graph-distances to assign a LBA state (Refine, Constant & Ignore) for each parameter (poses,
        ...
        // - the number of cameras to refine cannot be < to the number of newly added cameras (set to 'refine' by
        // default)
        if(_localBA_data->getNumOfRefinedPoses() > newReconstructedViews.size())
        {
            ...
            ALICEVISION_LOG_WARNING("The refinement has not been done: the new cameras are not connected to the rest "
                                    "of the local BA graph.");
    }
    ...
    std::string namecomplement =
        "_M" + std::to_string(kMinNbOfMatches) + "_D" + std::to_string(_localBA_data->getGraphDistanceLimit());
