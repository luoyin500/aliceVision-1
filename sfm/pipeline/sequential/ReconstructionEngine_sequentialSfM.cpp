...
#pragma omp critical
        {
            if(hasResected)
            {
                ...
                updateScene(viewId, newResectionData, i);
...
void ReconstructionEngine_sequentialSfM::updateScene(const IndexT viewIndex, const ResectionData& resectionData, int step)
{
    ...
    Vec3 trans = Vec3(step, 0, 0);
    trans(0) = step;
    const Pose3& initPoseI = Pose3(Mat3::Identity(), trans);
    const ResectionData reData;
    //reData.pose = initPoseI;
    ALICEVISION_LOG_INFO("Camera position: " << CameraPose(resectionData.pose).getTransform().translation().col(0)
                                             << CameraPose(initPoseI).getTransform().translation().col(1)
                                             << CameraPose(initPoseI).getTransform().translation().col(2));
