...
bool loadPairs(const std::string& sFileName, PairSet& pairs, int rangeStart, int rangeSize)
{
    ...
    for(; std::getline(in, sValue); ++nbLine)
    {
        ...
        for(size_t i = 1; i < str_size; ++i)
        {
            ...
            if(I == J)
            {
                ALICEVISION_LOG_WARNING("loadPairs: Invalid input file. Image " << I << " see itself. File: \""
                                                                                << sFileName << "\".");
                return false;
            }
            ...
            if(pairs.find(pairToInsert) != pairs.end())
            {
                ...
                ALICEVISION_LOG_WARNING("loadPairs: image pair (" << I << ", " << J << ") already added. File: \""
                                                                  << sFileName << "\".");
            }
